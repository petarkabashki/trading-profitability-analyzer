...
def simulate_trades_vectorized(num_trades, num_simulations, win_ratio, risk_reward_ratio, risk_per_trade_percent):
    ...
    Returns:
        ...
        - trade_simple_returns (list of lists): Simple trade returns for each simulation.
        - trade_returns (list of lists): Trade log returns for each simulation.
    """
    ...
    trade_simple_returns_simulation = np.where(wins, win_return, loss_return)
    ...
    trade_returns = np.where(wins, win_log_return, loss_log_return)

    # Cumulative log returns (each row = one simulation)
    cumulative_log_returns = np.cumsum(trade_returns, axis=1)
    ...
    return (cumulative_log_returns.tolist(),
            average_cumulative_log_returns,
            average_cumulative_log_return,
            trade_simple_returns_simulation.tolist(),
...
def plot_results(ax, cumulative_log_returns, average_cumulative_log_returns, num_simulations, win_ratio, risk_reward_ratio, num_trades, risk_per_trade_percent):
    """
    Plots the cumulative returns for each simulation and the average path.
    Input is now log returns, which are converted to simple returns for plotting.
    """
    ax.clear()
    colors = plt.cm.viridis(np.linspace(0, 1, num_simulations))
    for i, log_returns in enumerate(cumulative_log_returns):
        simple_returns = np.expm1(log_returns) # Convert log returns to simple returns
        ax.plot(np.array(simple_returns) * 100, color=colors[i], linewidth=0.5, alpha=0.3) # Convert to percentage for plotting
    average_simple_returns = np.expm1(average_cumulative_log_returns) # Convert log returns to simple returns for average
    ax.plot(average_simple_returns * 100, color='magenta', linewidth=2.5, label=f'Average ({num_simulations} simulations)') # Convert to percentage for plotting
...
def calculate_stats(average_trade_simple_results, average_trade_returns, average_cumulative_log_returns):
    """
    Computes performance statistics for the average simulation path.
    Input is now log returns for cumulative returns, and simple returns for individual trades.
    """
    simple_returns = np.array(average_trade_simple_results) # Use simple returns for gross profit/loss and profit factor
    log_returns = np.array(average_trade_returns) # Use log returns for Sharpe/Sortino ratio
    ...
    # Convert average_cumulative_log_returns to simple return for final return display
    final_simple_return = np.expm1(average_cumulative_log_returns[-1])
    max_drawdown = calculate_drawdown_log(average_cumulative_log_returns)

    return {
        "Final Return": f"{final_simple_return*100:.2f}%", # Display as percentage
...
if __name__ == "__main__":
    ...
    with st.spinner('Running simulations...'):
        (all_cumulative_log_returns, average_cumulative_log_returns_ts,
         average_cumulative_log_return, all_trade_simple_results, all_trade_returns) = simulate_trades_vectorized(
    ...
    plot_results(ax, all_cumulative_log_returns, average_cumulative_log_returns_ts, num_simulations, win_ratio, risk_reward_ratio, num_trades, risk_per_trade_percent)
    st.pyplot(fig)

    # --- Calculate and Display Stats Table ---
    average_trade_simple_results = np.nanmean(np.array(all_trade_simple_results), axis=0)
    average_trade_returns = np.nanmean(np.array(all_trade_returns), axis=0)
    average_stats = calculate_stats(average_trade_simple_results, average_trade_returns, average_cumulative_log_returns_ts)
